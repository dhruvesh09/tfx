...
class _PropertyOperator(_PlaceholderOperator):
  ...
  def __init__(self, key: str, custom_property: bool = False):
    super().__init__()
    self._key = key
  ...
  def encode(
    ...
    result.operator.artifact_property_op.custom_property = self._custom_property
    return result
...
class ArtifactPlaceholder(Placeholder):
  ...
  def custom_property(self, key: str):
    self._operators.append(_PropertyOperator(key, custom_property=True))
    return self
